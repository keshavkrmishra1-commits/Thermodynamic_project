...
# Leave the graph blank on startup by setting default to an empty list
materials = st.sidebar.multiselect("Select Materials to Plot", options=filtered_df['Material_Name'].unique(), default=[])

# Cap the maximum temperature range to 1400 K
t_range = st.sidebar.slider("Temperature Range (K)", min_value=200.0, max_value=1400.0, value=(298.0, 1400.0), step=1.0)
...
for i, mat in enumerate(materials):
    ...
    # Filter out negative Cp values so the graph never drops below the X-axis
    Cp = np.where(Cp < 0, np.nan, Cp)
...
fig.update_layout(
    ...
    yaxis=dict(rangemode="tozero"), # Locks the bottom of the Y-axis exactly at 0
    hovermode="x unified", 
...
with col3:
    if calc_mat:
        ...
        # Display "N/A" instead of negative numbers if it falls below 0
        if cp_calc < 0:
            st.metric(label=f"Computed C_p for {calc_mat} at {calc_temp} K", value="N/A (Below 0)")
        else:
